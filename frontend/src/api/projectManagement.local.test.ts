...
describe('projectManagement local mock store', () => {
  ...
  it('seeds supplier profile demo data with complete ontology assets', async () => {
    ...
    expect(detail.aiInsightRun?.scannedDocumentCount).toBe(12)
  ...
  it('seeds member profile demo data with complete ontology assets', async () => {
    ...
    expect(detail.aiInsightRun?.scannedDocumentCount).toBe(15)
  ...
  it('seeds omnichannel inventory demo data with complete ontology assets', async () => {
    const detail = await getProjectDetail('proj-2421')

    expect(detail.name).toBe('全渠道库存本体')
    expect(detail.documents.some((item) => item.name.includes('全渠道库存本体模型说明书'))).toBe(true)
    expect(detail.documents.some((item) => item.name.includes('履约分单决策训练样本'))).toBe(true)
    expect(detail.documents).toHaveLength(18)
    expect(detail.dataSources).toHaveLength(3)
    expect(detail.aiInsightRun?.id).toBe('ai-oci-001')
    expect(detail.aiInsightRun?.scannedDocumentCount).toBe(18)
    expect(detail.schemaConfig.entityTypes.some((item) => item.name === 'ProductSku')).toBe(true)
    expect(detail.schemaConfig.entityTypes.some((item) => item.name === 'FulfillmentDecision')).toBe(true)
    expect(detail.schemaConfig.relationTypes.some((item) => item.name === 'fulfilled_by_node')).toBe(true)
    expect(detail.schemaConfig.relationTypes.some((item) => item.name === 'triggers_transfer_task')).toBe(true)
    expect(detail.schemaConfig.skills.some((item) => item.id === 'sk-oci-003')).toBe(true)
    expect(detail.actions.some((item) => item.id === 'act-oci-002')).toBe(true)
    expect(detail.functions.some((item) => item.id === 'fn-oci-004')).toBe(true)
    expect(detail.runs.find((item) => item.id === 'run-oci-001')?.candidateEntityCount).toBe(80)
    expect(detail.runs.find((item) => item.id === 'run-oci-001')?.candidateRelationCount).toBe(66)
    expect(detail.versions.find((item) => item.id === 'ver-oci-001')?.entityCount).toBe(80)
    expect(detail.versions.find((item) => item.id === 'ver-oci-001')?.relationCount).toBe(66)
    expect(detail.currentVersionId).toBe('ver-oci-001')
  })
