...
import { BASE_MODEL_OPTIONS, REFRESHED_MODEL_NAMES, getModelDisplayName } from '../types/modelCatalog'
  ...
  return [
    {
      ...
      description: '基于 Deepexi2.0-60B-A14B 底座构建的工业问答与 SOP 推理增强项目',
      modelFamily: 'LLM',
      modality: 'text',
      trainStage: 'sft',
      capability: 'reasoning',
      datasetType: 'conversation',
      alignmentTags: ['工业知识增强', '长上下文', '底座复用'],
    ...
    },
    {
      ...
      description: '面向设备巡检图像问答与 grounded diagnosis 的多模态增强项目',
      modelFamily: 'VL',
      modality: 'image-text',
      trainStage: 'qlora',
    ...
    },
    {
      ...
      description: '面向票据、工单与检修文档解析的轻量多模态强化项目',
      modelFamily: 'VL',
      modality: 'image-text',
      trainStage: 'lora',
      ...
      trainMethod: 'lora',
      baseModel: 'Deepexi-VL2.0-7B-Doc',
    ...
    },
    {
      ...
      description: '基于 Deepexi2.0-60B-A14B 底座封装的通用对话与安全对齐项目',
      modelFamily: 'LLM',
      modality: 'text',
      trainStage: 'dpo',
      capability: 'chat',
      datasetType: 'preference',
      alignmentTags: ['安全对齐', '事实性增强', '底座复用'],
...
export async function startTraining(projectKey: string): Promise<TrainingJob> {
  ...
  const newJob: TrainingJob = {
    ...
    projectDisplayName: project?.displayName ?? project?.name ?? 'unknown-model',
    ...
    datasetName: project?.datasetName ?? '',
    baseModel: project?.baseModel ?? BASE_MODEL_OPTIONS[0],
...
export async function createTrainingProject(input: {
  ...
  const project: TrainingProject = {
    ...
    capability: input.baseModel?.includes('VL')
      ? (input.baseModel.includes('Doc') ? 'document-parsing' : 'vision-language-understanding')
      : 'chat',
    datasetType: 'conversation',
    ...
    baseModel: input.baseModel ?? BASE_MODEL_OPTIONS[0],
