...
vi.mock('./dataSource', () => ({
  listDataSources: vi.fn(async () => [
    { id: 'ds-un-1', name: '维修手册知识库' },
    { id: 'ds-un-2', name: '故障复盘报告库' },
  ]),
}))
...
describe('transform dataset-prep mock store', () => {
  ...
  it('seeds document-oriented dataset prep defaults', async () => {
    ...
    expect(list.some((item) => item.description.includes('文档')) || list.some((item) => item.description.includes('切片'))).toBe(true)
  ...
  it('migrates legacy ETL records to the new transform defaults', async () => {
    ...
    expect(list.some((item) => item.type === 'DocumentParsing')).toBe(true)
    expect(list.some((item) => item.description.includes('手册')) || list.some((item) => item.description.includes('报告'))).toBe(true)
    expect(mockStore._v).toBeDefined()
  })
  ...
  it('creates a dataset prep task with resolved datasource display names', async () => {
    ...
    expect(created.inputSourceNames).toEqual(['维修手册知识库', '故障复盘报告库'])
