...
产品流程：
一）基于本体的智能场景创建流程：
1、接入数据源（结构化、非结构化数据）->数据清洗->本体定义->本体结合数据生成本体实例（知识图谱）->Skllis生成->发布skills广场->多智能体编排及skill调用->发布智能体为数字员工；
二）训练本体意识的大模型流程：
...
| L4 | Co-worker平台 | Studio | Agent设计、Skills注册与管理、工具绑定、编排、部署 | AI Agent服务 |
...
以下为平台在一个 Space 下的完整目录树布局。每个 `[Project]` 是独立的权限边界，内部文件夹通过 `/output` 对外暴露标准化接口。
DeepexiOS v2.0:
├── 总览
│   ├── Title: 流水线全景
│   └── 平台健康度监控
